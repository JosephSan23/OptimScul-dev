...
export class GruposListaComponent implements OnInit {
  ...
  cupoTexto(g: Grupo): string {
    const usados = g.totalEstudiantes ?? 0;
    return g.cupoMaximo ? `${usados}/${g.cupoMaximo}` : `${usados}`;
  }

  cupoLleno(g: Grupo): boolean {
    return !!g.cupoMaximo && (g.totalEstudiantes ?? 0) >= g.cupoMaximo;
  }
}
